...
const INDEX_PAGE = 'List_of_National_Trust_for_Scotland_properties';
const HOST_ORG = 'National Trust for Scotland';
const NTS_HOSTNAME = 'nts.org.uk';

const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);
const geminiModel = genAI.getGenerativeModel({ model: 'gemini-2.5-flash' });

const WIKI_HEADERS = {
    'User-Agent': 'NationalTrustScotlandPropertiesBot/1.0 (contact@30x30project.org.uk; Academic/Environmental research crawler)'
};
...
function isValidPropertyPageTitle(title) {
    ...
    const exactIgnores = [
        "national trust for scotland", "national trust", "scotland", "england", "wales",
        "northern ireland", "united kingdom", "wayback machine", "wikipedia",
        "facebook", "twitter", "bbc", "heritage lottery fund",
        "national lottery heritage fund", "historic environment scotland",
        ...
        "category a listed building", "category b listed building",
    ];
...
function extractNTSUrl(wikitext) {
    if (!wikitext) return null;
    const urls = wikitext.match(/https?:\/\/[^\s\|\]\}\<"']+/ig) || [];
    for (let url of urls) {
        try {
            const hostname = new URL(url).hostname.toLowerCase().replace(/^www\./, '');
            if (hostname === NTS_HOSTNAME || hostname.endsWith('.' + NTS_HOSTNAME)) return url;
        } catch (e) {}
    }
    ...
    if (infoboxMatch) {
        try {
            ...
            if (hostname === NTS_HOSTNAME || hostname.endsWith('.' + NTS_HOSTNAME)) return infoboxMatch[1];
        } catch (e) {}
    }
    return null;
}
...
const NATURE_KEYWORDS = [
    ...
    'meadow', 'grassland', 'heath', 'heathland', 'moorland', 'moor',
    'munro', 'glen', 'loch', 'ben', 'mountain', 'highland',
    'coast', 'coastal', 'cliff', 'sand dune', 'beach',
    'wildlife', 'habitat', 'biodiversity', 'ecology', 'ecological',
    'conservation area', 'nature conservation',
    'flora', 'fauna', 'bat roost', 'bats', 'otter', 'red squirrel', 'deer', 'osprey', 'golden eagle', 'red kite',
    'ancient tree', 'veteran tree', 'orchard',
    'countryside', 'upland', 'lowland',
    'loch', 'burn', 'river', 'waterway', 'glen',
];
...
The National Trust for Scotland property is: "${name}"
...
Write a single concise sentence (max 30 words) that highlights the nature, landscape, or ecological value of this property — e.g. its woodlands, wildlife, coastline, meadows, highland terrain, or open countryside. Do not mention it is a National Trust for Scotland property. Do not mention history or architecture unless nature is entirely absent. If there is genuinely no nature angle, reply with exactly: SKIP`;
...
async function fetchImageUrls(fileTitles) {
    ...
    for (const chunk of chunks) {
        try {
            const data = await fetchWithCache('nts_imageinfo_v1', chunk.join('|'), async () => {
...
async function fetchBatchDetails(titles) {
    ...
    try {
        const responseData = await fetchWithCache('nts_query_v1', titles.join('|'), async () => {
        ...
        for (let id in pages) {
            ...
            // Must have NTS context
            const hasNTSContext =
                extractLower.includes('national trust for scotland') ||
                extractLower.includes('nts.org.uk') ||
                extractNTSUrl(wikitext) !== null;

            if (!hasNTSContext) continue;
            ...
            const locationUrl = extractNTSUrl(wikitext) || null;
            ...
            if (!descriptionText) descriptionText = 'A National Trust for Scotland property.';
...
async function extractPropertiesFromIndexHtml(htmlContent, skipNames = new Set()) {
    ...
    // Pull links from tables, lists, and columns on the index page
...
async function scrapeIndexPage(skipNames) {
    ...
    const responseData = await fetchWithCache('nts_parse_index_v1', INDEX_PAGE, async () => {
        const response = await axios.get(apiUrl, { params, headers: WIKI_HEADERS });
        return response.data;
    });
...
async function main() {
    console.log('🏴󠁧󠁢󠁳󠁣󠁴󠁿  National Trust for Scotland Properties Scraper — Wikipedia Source');
